<h1 align="center">Hi 👋, I'm Muhammad Tayyab</h1>
<h3 align="center">Full Stack Developer | AI Enthusiast | Problem Solver</h3>
<img align="right" alt="Coding" width="400" src="https://lottiefiles.com/free-animations/code /">

---

## 💫 About Me
- 💻 Full Stack Developer with experience in React.js, Node.js & Python  
- 🚀 Building scalable web applications & automation solutions  
- 🤖 Passionate about AI/ML and real-world problem solving  
- ☁️ Experience with cloud deployment & REST API development  
- 📍 Based in Pakistan  

---

## 🛠️ Tech Stack

### 💻 Languages
JavaScript (ES6+), TypeScript, Python, SQL, HTML5, CSS3  

### ⚛️ Frontend
React.js, Next.js, Tailwind CSS, Material UI, Bootstrap  

### 🔧 Backend
Node.js, Express.js, REST APIs  

### 🗄️ Databases
MongoDB, Firebase, PostgreSQL, SQLite, Oracle  

### 🤖 AI/ML
TensorFlow, Scikit-learn, CNN, Transfer Learning  

### ⚙️ Tools & DevOps
Git, GitHub, AWS (EC2), CI/CD, Postman, VS Code  

---

## 🔥 Featured Projects

### 🛍️ Velora Fashion Store
- Modern e-commerce frontend with Redux state management  
- Dynamic product handling with REST APIs  
- 🔗 Live: https://velora-fashion-store.vercel.app/  

### 🛒 Pickbazar E-commerce
- Scalable UI components & state management  
- Real-time API-based product rendering  
- 🔗 Live: https://pickbazar-ecommerce.vercel.app/  

### 🌦️ Weather App
- Real-time weather data using API integration  
- Clean UI with responsive design  
- 🔗 Live: https://weather-app-delta-seven-38.vercel.app/  

### 🏦 YourBank
- Responsive fintech-style banking interface  
- Clean UI/UX with Bootstrap  
- 🔗 Live: https://your-bank-responsive-banking-websit.vercel.app/  

### 🤖 Movie Recommendation System
- ML-based recommendation engine using Scikit-learn  
- Personalized suggestions using similarity algorithms  

### 🧠 Skin Virus Detection
- Deep learning model using CNN architectures  
- Transfer learning with VGG16, ResNet50  

---

## 📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=mtayyab9558&show_icons=true&theme=radical.gif" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=mtayyab9558&theme=radical" />
</p>

---

## 🌐 Connect With Me
- 💼 LinkedIn: https://linkedin.com/in/muhammad-tayyab1697/  
- 📧 Email: mtayyab9558@gmail.com  
- 💻 GitHub: https://github.com/mtayyab9558  

---

⭐ *"Building scalable solutions with clean code and modern technologies"*  
